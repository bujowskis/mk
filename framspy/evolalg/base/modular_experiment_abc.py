...
class ModularExperimentABC(ABC):
    ...
    def make_new_population(self, individuals, prob_mut, prob_xov, tournament_size):
        """
        Makes new population from the given individuals
        """
        pass
    ...
    def evolve(self, hof_savefile, generations, initialgenotype, pmut, pxov, tournament_size):
        ...
        if state is not None:  # loaded state from file
            ...
            print(
                ...
                len(self.population_structures.population), len(self.hof),
                len(self.stats), (len(self.population_structures.archive)), self.current_generation,
                generations))  # self.current_generation (and g) are 0-based, parsed_args.generations is 1-based
