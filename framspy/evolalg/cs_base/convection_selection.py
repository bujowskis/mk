# based on experiment_island_model.py
...
class ConvectionSelection(ConvectionSelectionABC, ABC):
    ...
    def set_state(self, state):
        ...
        for h in sorted(hof_, key=lambda
                x: x.rawfitness):  # sorting: ensure that we add from worst to best so all individuals are added to HOF
            self.hof.add(h)
    ...
    def evolve(self, hof_savefile, generations, initialgenotype, pmut, pxov, tournament_size):
        ...
        if state is not None:  # loaded state from file
            ...
            print(
                ...
                len(self.populations[0].population), len(self.hof), len(self.stats), (len(self.populations[0].archive)),
                self.current_genneration,
                generations))  # self.current_genneration (and g) are 0-based, parsed_args.generations is 1-based
        else:
