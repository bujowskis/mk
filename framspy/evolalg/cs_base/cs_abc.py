...
class ConvectionSelectionABC(ABC):
    current_population = []
    hof = []
    stats = []
    current_generation = 0  # change from current_genneration
    ...
    def add_genotype_if_valid(self, ind_list, genotype):
    ...
    def make_new_population(self, individuals, prob_mut, prob_xov, tournament_size):
        ...
        assert expected_mut + expected_xov <= N, "If probabilities of mutation (%g) and crossover (%g) added together exceed 1.0, then the population would grow every generation..." % (
        prob_mut, prob_xov)
        ...
        while len(newpop) < expected_mut:
            ...
            self.add_genotype_if_valid(newpop, self.mutate(ind.genotype))
        ...
        while len(newpop) < expected_mut + expected_xov:
            ...
            self.add_genotype_if_valid(newpop, self.cross_over(ind1.genotype, ind2.genotype))
    ...
    def save_state(self, state_filename):
        ...
        try:
            with open(state_filename_tmp, "wb") as f:
                pickle.dump(state, f)
            os.replace(state_filename_tmp,
                       state_filename)  # ensures the new file was first saved OK (e.g. enough free space on device), then replace
        except Exception as ex:
            raise RuntimeError(
                "Failed to save evolution state '%s' (because: %s). This does not prevent the experiment from continuing, but let's stop here to fix the problem with saving state files." % (
    ...
    def update_stats(self, generation, all_individuals):
        ...
        self.hof.add(
            best)  # instead of single best, could add all individuals in population here, but then the outcome would depend on the order of adding
