...
def cec2017_numerical_mutation(genotype, standard_deviation: float = 0.5):  # fixme - sd value
    """
    Changes all dimensions with displacement according to normal distribution
    """
    raw_individual = [genotype[i] + np.random.normal(0, standard_deviation) for i in range(len(genotype))]
