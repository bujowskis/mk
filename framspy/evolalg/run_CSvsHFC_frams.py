...
from evolalg.numerical_CSvsHFC.frams_cs import ExperimentFramsCSEquiwidth
...
def main():
    ...
    hof, stats, df = experiment.evolve(hof_savefile=f'HoF/numerical_CSvsHFC/framsticks/HoF-frams-{sim_file}-{repetition}-{fraction}-{migration_interval}-{number_of_populations}-{subpopsize}-{pmut}-{pxov}-{tournament_size}.gen',
                    generations=generations,
                    initialgenotype=parsed_args.initialgenotype,
                    pmut=pmut,
                    pxov=pxov,
                    tournament_size=tournament_size)
    df.to_csv(f'results/numerical_CSvsHFC/framsticks/numerical_CSvsHFC-frams-{sim_file}-{repetition}-{fraction}-{migration_interval}-{number_of_populations}-{subpopsize}-{pmut}-{pxov}-{tournament_size}.csv')
