...
def main():
    ...
    hof, stats, df = experiment.evolve(hof_savefile=f'HoF/frams/cs/frams_HoF-cs-{sim_file}-{repetition}-{migration_interval}-{number_of_populations}-{subpopsize}-{pmut}-{pxov}-{tournament_size}.gen',
    ...
    df.to_csv(f'results/frams/cs/frams_CSvsHFC-cs-{sim_file}-{repetition}-{migration_interval}-{number_of_populations}-{subpopsize}-{pmut}-{pxov}-{tournament_size}.csv')
